...
class SFTDatasetV2V(Dataset):
    def __init__(self, data_dir, video_size, fps, max_num_frames, skip_frms_num=3):
        """
        skip_frms_num: ignore the first and the last xx frames, avoiding transitions.
        """
        super(SFTDatasetV2V, self).__init__()
        ...
        for root, dirnames, filenames in os.walk(data_dir):
            for filename in filenames:
                if filename.endswith("ref.mp4"): #### 这里进行了修改
                    video_path = os.path.join(root, filename)
                    self.video_paths.append(video_path)

                    caption_path = video_path.replace("ref.mp4", ".txt").replace("videos", "labels")
                    vid_cond_path = video_path.replace("ref", "cond")
                    self.vid_cond_paths.append(vid_cond_path)
                    if os.path.exists(caption_path):
                        caption = open(caption_path, "r").read().splitlines()[0]
                    else:
                        caption = ""
    ...
    def __getitem__(self, index):
        ...
        video_path = self.video_paths[index]
        cond_path=self.vid_cond_paths[index]
        def path2vid(video_path):
            vr = VideoReader(uri=video_path, height=-1, width=-1)
            actual_fps = vr.get_avg_fps()
            ori_vlen = len(vr)
            ...
            tensor_frms = resize_for_rectangle_crop(tensor_frms, self.video_size, reshape_mode="center")
            tensor_frms = (tensor_frms - 127.5) / 127.5
            return tensor_frms, num_frames
        
        tensor_frms,num_frames = path2vid(video_path)
        cond_tensor_frms, _ = path2vid(cond_path)

        item = {
            "mp4": tensor_frms,
            "mp4_cond": cond_tensor_frms,
            "txt": self.captions[index],
            "num_frames": num_frames,
            "fps": self.fps,
        }
